...
def main():
    parser = argparse.ArgumentParser(description="Render a camera path with Nerfstudio.")
    parser.add_argument("--load-config", type=Path, required=True, help="Path to config YAML file.")
    parser.add_argument("--camera-path-filename", type=Path, required=True, help="Filename of the camera path to render.")
    parser.add_argument("--output-path", type=Path, required=True, help="Path to output video file.")
    parser.add_argument("--output-format", type=str, choices=["images", "video"], default="video", help="How to save output data.")
    parser.add_argument("--image-format", type=str, choices=["jpeg", "png"], default="jpeg", help="Image format")
    parser.add_argument("--jpeg-quality", type=int, default=100, help="JPEG quality")
    parser.add_argument("--downscale-factor", type=float, default=1.0, help="Scaling factor to apply to the camera image resolution.")
    parser.add_argument("--eval-num-rays-per-chunk", type=int, default=None, help="Specifies number of rays per chunk during eval. If None, use the value in the config file.")
    parser.add_argument("--rendered-output-names", nargs='+', default=["rgb"], help="Name of the renderer outputs to use. rgb, depth, etc. concatenates them along y axis")
    parser.add_argument("--depth-near-plane", type=float, default=None, help="Closest depth to consider when using the colormap for depth. If None, use min value.")
    parser.add_argument("--depth-far-plane", type=float, default=None, help="Furthest depth to consider when using the colormap for depth. If None, use max value.")
    parser.add_argument("--render-nearest-camera", action='store_true', help="Whether to render the nearest training camera to the rendered camera.")
    parser.add_argument("--check-occlusions", action='store_true', help="If true, checks line-of-sight occlusions when computing camera distance and rejects cameras not visible to each other.")
    args = parser.parse_args()

    render_camera_path = RenderCameraPath(
        ...
        depth_near_plane=args.depth_near_plane,
        depth_far_plane=args.depth_far_plane,
        render_nearest_camera=args.render_nearest_camera,
        check_occlusions=args.check_occlusions,
    )
